...
function genComplexity() {
  const items = [
    {
      q: '要素数 n の配列を先頭から順に1回走査する処理の計算量として，適切なものはどれか。',
      a: 'O(n)',
      wrong: ['O(1)', 'O(n²)', 'O(log n)'],
      exp: '配列全体を1回走査するので線形時間 O(n)。',
    },
    {
      q: '要素数 n の整列済み配列に対する2分探索の計算量として，適切なものはどれか。',
      a: 'O(log n)',
      wrong: ['O(n)', 'O(n²)', 'O(1)'],
      exp: '探索範囲が半分になるので O(log n)。',
    },
    {
      q: '二重ループで、外側・内側とも 1〜n を回る処理の計算量として，適切なものはどれか。',
      a: 'O(n²)',
      wrong: ['O(n)', 'O(log n)', 'O(n log n)'],
      exp: 'n×n 回なので O(n²)。',
    },
    {
      q: '配列の先頭要素を参照する処理の計算量として，適切なものはどれか。',
      a: 'O(1)',
      wrong: ['O(n)', 'O(log n)', 'O(n²)'],
      exp: 'インデックスで直接アクセスできるので定数時間 O(1)。',
    },
    {
      q: 'マージソートの平均計算量として，適切なものはどれか。',
      a: 'O(n log n)',
...
const SECURITY_BANK = [
  {
    q: '公開鍵暗号方式の説明として，適切なものはどれか。',
    a: '暗号化と復号で異なる鍵を使う',
    wrong: ['暗号化と復号で同じ鍵を使う', '暗号化だけに鍵が必要', '復号だけに鍵が必要'],
    hint: '公開鍵と秘密鍵のペアを使います。',
    exp: '公開鍵暗号は暗号化鍵と復号鍵が異なる（非対称暗号）。共通鍵暗号は同じ鍵を使う。',
  },
  {
    q: 'ディジタル署名の主な目的として，適切なものはどれか。',
    a: '改ざん検知と送信者の認証',
  ...
  {
    q: 'ファイアウォールの基本的な役割として，適切なものはどれか。',
    a: '許可されていない通信を遮断する',
  ...
  {
    q: 'HTTPSで主に用いられるプロトコルの組合せとして，適切なものはどれか。',
  ...
  {
    q: 'ソーシャルエンジニアリングの説明として，適切なものはどれか。',
    a: '人の心理的な隙を突いて情報を得る手法',
    wrong: ['OSの弱点だけを突く攻撃', '暗号の数学的解読だけを指す', '装置故障を起こす手法'],
    hint: '技術ではなく人を標的にします。',
    exp: '電話・メール等で信頼を装い機密情報を聞き出すなど。',
  },
  {
    q: 'ランサムウェアの特徴として，適切なものはどれか。',
  ...
  {
    q: 'WAF（Web Application Firewall）の役割として，適切なものはどれか。',
